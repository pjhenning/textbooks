...
export function setupHourglass(
  ...
  $slider.on('move', n => {
    const percent = n / 100;
    toDrain.shape.setFillAmount((toDrain.startAmount ?? 1) - (percent * toDrain.drainAmount));
    toFill.setFillAmount(percent);
    $solid.scene.draw();
  });
}
...
export class WaterSphere extends WaterShape {
  ...
  private heightPercentForFilledPercent(filledPercent: number) {
    return (Math.asin((2 * filledPercent) - 1) / Math.PI) + 0.5;
  }
